...
int main()
{
    ...
    cout << "InitDataCost took " << chrono::duration_cast<chrono::milliseconds>(end-start).count() << "ms" << endl;
    for(int i=0; i < BP_ITERATIONS; i++) {
        ...
        cout << "iteration " << (i+1) << "/" << BP_ITERATIONS << ", energy = " << energy << ", took " << chrono::duration_cast<chrono::milliseconds>(end-start).count() << "ms" << endl;
    }
